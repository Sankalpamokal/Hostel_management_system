...
<div id="radio" style="padding-bottom: 20px">
        <input type="radio" name="source" value="allocate" id="allocate" onclick="javascript:window.location.href='allocation.php'; return false;" checked >Allocation/Reject   
          <input type="radio" name="source" value="dealloacte" id="dealloacte" onclick="javascript:window.location.href='deallocation.php'; return false;" >Deallocation
...
<form method="post" action="showstudentdata.php">
<!--<input type="number" name="appid" placeholder="Enter student's Id"> -->
<select name="appid" id="appid" required>
  <?php

$appid=mysqli_query($conn,"select DISTINCT appid from application" );
while($row=mysqli_fetch_array($appid))
{	
?>
<option><?php echo $row["appid"]; ?></option>
<?php
}
?>	
        </select><br/>
