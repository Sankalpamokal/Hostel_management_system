...
<div id="radio" style="padding-bottom: 20px">
        <input type="radio" name="source" value="allocate" id="allocate" onclick="javascript:window.location.href='allocation.php'; return false;"  >Allocation/Reject  
          <input type="radio" name="source" value="dealloacte" id="deallocate" onclick="javascript:window.location.href='deallocation.php'; return false;" checked>Deallocation
          <input type="radio" name="source" value="sinfo" id="sinfo" onclick="javascript:window.location.href='sinfo.php'; return false;">Student Info
        </div>
<form method="post" action="deallocationdata.php">
<select name="roomno" id="roomno" required>
  <?php

$aroomno=mysqli_query($conn,"select DISTINCT aroomno from allocate" );
while($row=mysqli_fetch_array($aroomno))
{	
?>
<option><?php echo $row["aroomno"]; ?></option>
<?php
}
?>	
        </select>
<br/>
<br/>
<input type="submit" name="allocate" value="Deallocate" >
</form>
</div>
<div><br/>
<h2 style="text-align:center;font-size:2vw;color:white;">Applications Allocated</h2>
<?php
include_once("config.php"); 

$sql = "SELECT allocatedid, fname, mobile, gender, aroomno FROM allocate";
