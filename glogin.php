...
window.location='glogin.php';
</script>";//You will be sent to Login.php for re-login 
}//You will be sent to Login.php for re-login 
} 
$inUsername = $_POST["username"]; // as the method type in the form is "post" we are using $_POST otherwise it would be $_GET[] 
$inPassword = $_POST["password"]; 
$stmt= $conn->prepare("SELECT USERNAME, EMAIL, PASSWORD FROM WATCHMAN WHERE EMAIL = ?"); //Fetching all the records with input credentials
...
$_SESSION['email1']=$EmailDB;
$_SESSION['password']=$PasswordDB;
header("location: wentry.php"); // user will be taken to profile page
}
else
{
...
<div id="radio" style="padding-bottom: 20px">
        <input type="radio" name="source" value="student" id="student" onclick="javascript:window.location.href='login.php'; return false;"  >Student   
          <input type="radio" name="source" value="admin" id="admin" onclick="javascript:window.location.href='alogin.php'; return false;" >Admin
          <input type="radio" name="source" value="watchman" id="watchman" onclick="javascript:window.location.href='glogin.php'; return false;" checked>Watchman
        </div>
				<div class="inputbox">
