...
<h2 style="font-size: 2vw; margin-top: 15px;margin-bottom: 20px;color: white;">Student Application</h2>
...
<input type="text" name="econtact" value="<?php echo $econtact ?>" id="econtact" required="required">

<label>Guardian  Name : </label>
<input type="text" name="gname" id="gname" value="<?php echo $gname ?>"required="required">


<label>Guardian  Relation : </label>
<input type="text" value="<?php echo $grelation ?>" name="grelation" id="grelation"  required="required">


<label>Guardian Contact no : </label>
<input type="text" name="gcontact" value="<?php echo $gcontact ?>" id="gcontact" required="required">
    <br/>
	
		<h2>Correspondance Address</h2>
<label>Address : </label>
<input type="text" name="address" value="<?php echo $address ?>"  id="address" required>

<label>City : </label>
<input type="text" name="city" value="<?php echo $city ?>" id="city" required="required">
...
<label>Pincode : </label>
<input type="text" name="pincode" value="<?php echo $pincode ?>" id="pincode" required="required">
...
<?php  
    ...
    Hostel Building:<br>
      <input type="text" name="hbuilding" value="<?php echo $hbuilding ?>">
        <br><br>
    Floor Number:<br>
    ...
    Type of room:
	<input type="text" name="room" value="<?php echo $room ?>"><br/>

	Ac/Non-Ac:
	<input type="text" name="ac" value="<?php echo $ac ?>"><br/>
	Food:
	<input type="text" name="food" value="<?php echo $food ?>"><br/>
	
		<h1 style="font-size:2vw; text-align:center;">Admin Section</h1><hr>

		<h2>Building Info</h2>
 <!--   Building Number:
    <select name="ahbuilding" required>
  ...
  <br> -->
  Room No:
  <select name="aroomno" id="aroomno">
  <?php

$aroomno=mysqli_query($conn,"select DISTINCT roomno from addroom where allocated='No' AND gender='$gender' AND hbuilding='$hbuilding' AND floorno='$fno' AND room='$room'");
while($row=mysqli_fetch_array($aroomno))
{	
?>
...
?>	
  ...
  <br>
  AC Feature:
  <select name="aac" required>
  <option value="AC">With AC</option>
  <option value="Without_AC">Without AC</option>
  </select>
  <br>
  Food Services:
  <select name="afood" required>
  <option value="With_Food(Veg)">With Food(Veg)</option>
  <option value="With_Food(Non-Veg)">With Food(Non-Veg)</option>
  <option value="Without_Food">Without Food</option>
</select>
  <br>

    <button type="submit" name="allocate" value="Allocate" formaction="allocatedata.php" >Allocate</button>
	<button type="submit" name="reject" value="Reject" formaction="rejectdata.php">Reject</button>
    </form>
